...
def QPM(X0: np.ndarray, max_iter=1000):
    ...
    while iter_index < max_iter:
        ...
        for i in range(cons.cons_len):
            H = H + active_flag[i] * mu_k * np.outer(cons.ai[i].T, cons.ai[i])
            B = B + active_flag[i] * mu_k * cons.b[i] * cons.ai[i].T
        # 求解当前状态下的近似解
        xk = -np.dot(np.linalg.inv(H), B)
        # 计算当前位置梯度
        # grad = np.dot(H.T, xk) + B
        # if np.linalg.norm(grad) < 1e-5:
        #     break
        # 更新约束激活标志位
        active_flag = ((np.dot(cons.ai, xk) + cons.b) < np.ones(cons.cons_len) * (-1e-20)).astype(np.int)
        print(f'iter times:{iter_index}, xk: {xk}, active flag: {active_flag}')
